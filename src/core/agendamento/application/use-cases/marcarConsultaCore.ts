...
export type MarcarConsultaCoreInput = {
  clinicaId: string;
  pacienteId: string;
...
/**
 * Núcleo de marcação — disponibilidade, sobreposição, persistência e lembrete.
 * Sem RBAC / sessão. Usado pelo painel e pelo canal `link-publico`.
 */
export class MarcarConsultaCore {
  constructor(
  ...
  async executar(input: MarcarConsultaCoreInput): Promise<Agendamento> {
